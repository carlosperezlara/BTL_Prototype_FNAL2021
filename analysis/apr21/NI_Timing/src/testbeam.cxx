...
//=================================
void testbeam::CalibrateTime( double A[7], double B[7], double C[7], double X[7], double Y[7], double Z[7] ) {
  {
    double L0[7] = { -1.65243, -1.54752, -1.54752, -1.54752, -1.33236, -1.21982, -1.05165 };
    double L1[7] = { 35.7638, 41.7519, 41.7519, 41.7519, 54.8656, 65.3966, 89.4069 };
    double L2[7] = { -5.24402, -6.8977, -6.8977, -6.8977, -10.9248, -12.9603, -16.0445 };
    double R0[7] = { -1.32913, -1.21708, -1.21708, -1.21708, -1.03013, -0.927725, -0.768945 };
    double R1[7] = { 32.4291, 36.7979, 36.7979, 36.7979, 52.9871, 64.8151, 89.3923 };
    double R2[7] = { -28.8281, -33.1812, -33.1812, -33.1812, -40.8888, -46.1654, -55.0166 };
    for(int i=0; i!=7; ++i) {
  ...
  if(fRun==44734) {
    double L0[7] = { -1.6615, -1.55449, -1.47598, -1.42324, -1.3406, -1.2321, -1.06282 };
    double L1[7] = { 36.4287, 42.9083, 46.8782, 50.4845, 56.6755, 68.0127, 92.0804 };
    double L2[7] = { -4.07766, -6.34482, -7.8832, -8.74751, -10.2309, -12.2931, -15.2334 };
    double R0[7] = { -1.32864, -1.22296, -1.15261, -1.10687, -1.03746, -0.936974, -0.779905 };
    double R1[7] = { 31.1966, 37.4764, 42.1216, 46.9352, 54.5592, 66.899, 91.892 };
    double R2[7] = { -27.3957, -32.639, -35.1974, -36.1975, -38.2084, -43.3423, -51.9555 };
    for(int i=0; i!=7; ++i) {
      A[i] = L0[i]; B[i] = L1[i]; C[i] = L2[i];
      X[i] = R0[i]; Y[i] = R1[i]; Z[i] = R2[i];
    }
  ...
  if(fRun==44733) {
    double L0[7] = { -1.64547, -1.54668, -1.4721, -1.42314, -1.34304, -1.23095, -1.05994 };
    double L1[7] = { 33.8142, 40.807, 44.4796, 49.3381, 56.0258, 67.0524, 91.1824 };
    double L2[7] = { -11.3535, -11.9435, -13.1117, -13.7659, -14.8081, -16.5593, -19.6005 };
    double R0[7] = { -1.38046, -1.27654, -1.21445, -1.16362, -1.08595, -0.982456, -0.818744 };
    double R1[7] = { 33.1155, 38.7656, 46.3957, 50.6308, 56.9024, 68.9559, 92.6243 };
    double R2[7] = { -26.3077, -29.5674, -32.911, -35.0622, -38.9338, -44.2278, -53.9192 };
    for(int i=0; i!=7; ++i) {
      A[i] = L0[i]; B[i] = L1[i]; C[i] = L2[i];
      X[i] = R0[i]; Y[i] = R1[i]; Z[i] = R2[i];
    }
  ...
  if(fRun==44731) {
    double L0[7] = { -1.64541, -1.53755, -1.46639, -1.41449, -1.33519, -1.22158, -1.05552 };
    double L1[7] = { 29.7185, 35.4395, 40.0993, 44.7681, 51.5636, 63.0823, 88.8249 };
    double L2[7] = { -9.08292, -10.3813, -11.4112, -11.859, -12.6482, -14.4194, -16.588 };
    double R0[7] = { -1.4289, -1.34166, -1.25934, -1.2079, -1.13512, -1.03496, -0.875409 };
    double R1[7] = { 43.1704, 51.5544, 52.5929, 56.8903, 64.2704, 77.3883, 102.482 };
    double R2[7] = { -26.1771, -28.042, -32.3069, -34.476, -37.7475, -42.0851, -50.6637 };
    for(int i=0; i!=7; ++i) {
      A[i] = L0[i]; B[i] = L1[i]; C[i] = L2[i];
      X[i] = R0[i]; Y[i] = R1[i]; Z[i] = R2[i];
    }
  ...
  if(fRun==44727) {
    double L0[7] = { -1.59093, -1.48553, -1.41682, -1.3626, -1.28018, -1.17286, -1.01595 };
    double L1[7] = { 18.7316, 26.8965, 32.1544, 35.9477, 42.8847, 55.3067, 85.1561 };
    double L2[7] = { -12.3549, -15.25, -16.0108, -16.8906, -17.9131, -19.4332, -22.2648 };
    double R0[7] = { -1.37605, -1.33322, -1.26293, -1.2116, -1.13244, -1.03169, -0.877962 };
    double R1[7] = { 26.5491, 45.0795, 48.9262, 52.5811, 59.0508, 71.7572, 98.4953 };
    double R2[7] = { -29.9394, -22.3742, -24.6739, -26.0125, -29.6799, -33.6742, -40.9186 };
    for(int i=0; i!=7; ++i) {
      A[i] = L0[i]; B[i] = L1[i]; C[i] = L2[i];
      X[i] = R0[i]; Y[i] = R1[i]; Z[i] = R2[i];
    }
  ...
  if(fRun==44725) {
    double L0[7] = { -1.64796, -1.53949, -1.46937, -1.4353, -1.35431, -1.24108, -1.06801 };
    double L1[7] = { 35.2058, 39.84, 44.2114, 50.3183, 57.5542, 69.6659, 95.5561 };
    double L2[7] = { -11.8458, -12.7539, -13.973, -8.65336, -10.0198, -12.3995, -16.0901 };
    double R0[7] = { -1.44535, -1.2948, -1.23744, -1.17642, -1.10349, -1.00196, -0.84767 };
    double R1[7] = { 48.4317, 45.3528, 51.2779, 55.6655, 63.1678, 75.8566, 102.379 };
    double R2[7] = { -26.9753, -40.0936, -37.2925, -40.582, -44.3937, -48.8061, -57.0068 };
    for(int i=0; i!=7; ++i) {
      A[i] = L0[i]; B[i] = L1[i]; C[i] = L2[i];
      X[i] = R0[i]; Y[i] = R1[i]; Z[i] = R2[i];
    }
  ...
  if(fRun==44730) {
    double L0[7] = { -1.61956, -1.52283, -1.4445, -1.38623, -1.30534, -1.19561, -1.02187 };
    double L1[7] = { 27.5273, 34.4868, 36.2052, 40.0304, 47.0543, 59.3114, 86.4421 };
    double L2[7] = { -12.399, -12.5952, -13.1541, -14.2397, -15.4158, -16.932, -20.0321 };
    double R0[7] = { -1.44575, -1.34475, -1.26873, -1.21698, -1.13881, -1.03025, -0.865416 };
    double R1[7] = { 48.2708, 52.0607, 55.6645, 59.7024, 67.2919, 78.9337, 104.814 };
    double R2[7] = { -27.9649, -30.4928, -34.27, -36.7041, -40.8568, -46.7984, -56.4907 };
    for(int i=0; i!=7; ++i) {
      A[i] = L0[i]; B[i] = L1[i]; C[i] = L2[i];
      X[i] = R0[i]; Y[i] = R1[i]; Z[i] = R2[i];
    }
  ...
  if(fRun==44728) {
    double L0[7] = { -1.65827, -1.53412, -1.47788, -1.42591, -1.34304, -1.23251, -1.06389 };
    double L1[7] = { 34.4558, 36.7775, 44.6603, 49.3107, 56.795, 69.3971, 98.1578 };
    double L2[7] = { -7.58909, -9.42325, -10.164, -10.8836, -12.3451, -14.4664, -17.3687 };
    double R0[7] = { -1.43455, -1.3303, -1.2448, -1.19463, -1.12386, -1.02396, -0.869391 };
    double R1[7] = { 40.0935, 46.7001, 45.9931, 49.7729, 58.5319, 72.0022, 101.853 };
    double R2[7] = { -26.9647, -30.5536, -35.3556, -37.4601, -40.6746, -45.7395, -55.3549 };
    for(int i=0; i!=7; ++i) {
      A[i] = L0[i]; B[i] = L1[i]; C[i] = L2[i];
      X[i] = R0[i]; Y[i] = R1[i]; Z[i] = R2[i];
    }
  ...
  if(fRun==44732) {
    double L0[7] = { -1.80714, -1.6094, -1.51838, -1.41704, -1.33671, -1.22502, -1.0444 };
    double L1[7] = { 67.9812, 54.8817, 54.7075, 47.3852, 54.8834, 67.8156, 94.8304 };
    double L2[7] = { -4.10835, -9.38831, -10.9371, -13.1056, -14.1725, -15.3453, -18.4579 };
    double R0[7] = { -1.44976, -1.3108, -1.25217, -1.20445, -1.13305, -1.01387, -0.848727 };
    double R1[7] = { 48.6344, 49.1138, 55.5358, 61.5454, 68.8025, 78.1488, 105.9 };
    double R2[7] = { -28.6898, -36.68, -38.6381, -41.0597, -44.037, -50.8865, -61.1966 };
    for(int i=0; i!=7; ++i) {
      A[i] = L0[i]; B[i] = L1[i]; C[i] = L2[i];
      X[i] = R0[i]; Y[i] = R1[i]; Z[i] = R2[i];
    }
...
*/
    ...
    double thresholds[7] = {-5,-10,-15,-20,-30,-50,-100};
    double L_P0[7];
    double L_P1[7];
    double L_P2[7];
    double R_P0[7];
